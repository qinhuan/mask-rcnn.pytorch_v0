...
class Generalized_RCNN(nn.Module):
    ...
    def roi_feature_transform(self, blobs_in, rpn_ret, blob_rois='rois', method='RoIPoolF',
        ...
        if isinstance(blobs_in, list) and not cfg.FPN.ADA_POOL:
        ...
        elif isinstance(blobs_in, list) and cfg.FPN.ADA_POOL:
            # FPN case: add RoIFeatureTransform to each FPN level
            device_id = blobs_in[0].get_device()
            k_max = cfg.FPN.ROI_MAX_LEVEL  # coarsest level of pyramid
            k_min = cfg.FPN.ROI_MIN_LEVEL  # finest level of pyramid
            ...
            for lvl in range(k_min, k_max + 1):
                ...
                sc = spatial_scale[k_max - lvl]  # in reversed order
                tmp_out_list = []
                for lvl_ in range(k_min, k_max + 1):
                    bl_rois = blob_rois + '_fpn' + str(lvl_)
                    if len(rpn_ret[bl_rois]):
                        rois = Variable(torch.from_numpy(rpn_ret[bl_rois])).cuda(device_id)
                        if method == 'RoIAlign':
                            xform_out = RoIAlignFunction(
                                resolution, resolution, sc, sampling_ratio)(bl_in, rois)
                            tmp_out_list.append(xform_out)
                xform_shuffled = torch.cat(tmp_out_list, dim=0)
                bl_out_list.append(xform_shuffled)

            xform_out_list = []
            for i in range(4):
                xform_shuffled = bl_out_list[i]
                # Unshuffle to match rois from dataloader
                device_id = xform_shuffled.get_device()
                restore_bl = rpn_ret[blob_rois + '_idx_restore_int32']
                restore_bl = Variable(
                    torch.from_numpy(restore_bl.astype('int64', copy=False))).cuda(device_id)
                xform_out = xform_shuffled[restore_bl]
                xform_out_list.append(xform_out)
            return xform_out_list     
            # B, C, H, W = xform_out.size()
            # xform_out_ = xform_out.view(1, B, C, H, W)
            # tmp_out_list.append(xform_out)
            # xform_out_cat = torch.cat(tmp_out_list, dim=0) # 4,b,c,h,w
            # xform_out_max = torch.max(xform_out_cat, 0)[0]
