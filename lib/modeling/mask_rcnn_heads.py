...
class mask_rcnn_with_boundary_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        mask_conv_list = []
        for i in range(4):
            mask_conv_list.extend([
                nn.Conv2d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation=dilation),
                nn.ReLU(inplace=True)
            ])
        self.conv3x3_mask = nn.Sequential(*mask_conv_list)

        self.deconv_mask = nn.ConvTranspose2d(dim_inner, dim_inner, 2, 2, 0)

        self.conv1x1_mask = nn.Conv2d(dim_in * 2, n_classes, 1, 1, 0)

        # boundary
        boundary_conv_list = []
        for i in range(4):
            boundary_conv_list.extend([
                nn.Conv2d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation=dilation),
                nn.ReLU(inplace=True)
            ])
        self.conv3x3_boundary = nn.Sequential(*mask_conv_list)

        self.deconv_boundary = nn.ConvTranspose2d(dim_inner, dim_inner, 2, 2, 0)

        self.conv1x1_boundary = nn.Conv2d(dim_in, n_classes, 1, 1, 0)
    ...
    def forward(self, x, rpn_ret):
        ...
        x_mask = self.conv3x3_mask(x)
        x_mask = self.deconv_mask(x_mask)

        x_boundary = self.conv3x3_boundary(x)
        x_boundary = self.deconv_boundary(x_boundary)

        x_m_b = torch.cat((x_mask, x_boundary), 1)
        x_m_b = F.relu(x_m_b, inplace=True)

        x_m_b = self.conv1x1_mask(x_m_b)
        x_boundary = self.conv1x1_boundary(x_boundary)
