...
class fpn(nn.Module):
    ...
    def __init__(self, conv_body_func, fpn_level_info, P2only=False):
        ...
        if cfg.FPN.PANet and cfg.FPN.FPN_ON:
            self.bottomup_lateral_modules = nn.ModuleList()
            for i in range(self.num_backbone_stages - 1):
                self.bottomup_lateral_modules.append(
                    bottomup_lateral_module(fpn_dim, fpn_dim)
                )
        #
    ...
    def forward(self, x):
        ...
        if cfg.FPN.PANet:
            # [2,3,4,5]
            pan_output_blobs = [fpn_output_blobs[-1]]
            for i in range(self.num_backbone_stages - 1):
                pan_output_blobs.append(
                    self.bottomup_lateral_modules[i](pan_output_blobs[-1], fpn_output_blobs[2-i])
                )
            fpn_output_blobs = pan_output_blobs
            fpn_output_blobs.reverse()  # [5,4,3,2]
...
class bottomup_lateral_module(nn.Module):
    """Add a bottom-up lateral module."""
    def __init__(self, dim_in_top, dim_in_lateral):
        super().__init__()
        self.conv_downsample = nn.Conv2d(dim_in_top, dim_in_top, 3, 2, 1)
        self.conv_posthoc = nn.Conv2d(dim_in_top, dim_in_top, 3, 1, 1)
        self._init_weights()

    def _init_weights(self):
        mynn.init.XavierFill(self.conv_downsample.weight)
        init.constant(self.conv_downsample.bias, 0)
        mynn.init.XavierFill(self.conv_posthoc.weight)
    ...
    def forward(self, down_blob, lateral_blob):
        ...
        return out # or F.relu(out, inplace=True)
