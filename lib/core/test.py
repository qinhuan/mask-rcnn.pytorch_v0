...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if cfg.MODEL.BOUNDARY_ON:
            cls_boundary = segm_results(cls_boxes, boundary, boxes, im.shape[0], im.shape[1])
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    if cfg.MODEL.BOUNDARY_ON:
        pred_masks, pred_boundary = model.module.mask_net(blob_conv, inputs)
        pred_masks = pred_masks.data.cpu().numpy().squeeze()
        pred_boundary = pred_boundary.data.cpu().numpy().squeeze()
        if cfg.MRCNN.CLS_SPECIFIC_MASK:
            pred_masks = pred_masks.reshape([-1, cfg.MODEL.NUM_CLASSES, M, M])
            pred_boundary = pred_boundary.reshape([-1, cfg.MODEL.NUM_CLASSES, M, M])
        else:
            pred_masks = pred_masks.reshape([-1, 1, M, M])
            pred_boundary = pred_boundary.reshape([-1, 1, M, M])
        return pred_masks, pred_boundary
    else:
